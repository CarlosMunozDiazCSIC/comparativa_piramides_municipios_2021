let firstItem = 'Total Nacional', secondItem = ''; //Variables de selección
...
function initData() {
    d3.queue()
        .defer(d3.csv, "./data/piramides_municipios.csv")
        .defer(d3.csv, "./data/piramides_nacional_ccaa_prov.csv")
        ...
        if (error) throw error;
            ...
            let data_global = d3.nest()
                .key(function(d) { return d.NAME; })
                .entries(data2);

            /////////
            ///////// Autocompletado
            let optGroupNac = document.createElement('OPTGROUP');
            optGroupNac.label = 'Provincias/CCAA/Nacional';

            for(let i = 0; i < data_global.length; i++) {
                let elem = data_global[i].key;
                let elemDom = document.createElement('option');
                elemDom.text = elem;

                optGroupNac.appendChild(elemDom);
            }
            let dupNac = optGroupNac.cloneNode(true);

            let optGroupMun = document.createElement('OPTGROUP');
            optGroupMun.label = 'Municipios';
            ...
            document.getElementById('listMunicipiosFirst').value = 'Total Nacional';
            document.getElementById('municipiosFirst').append(optGroupNac);
            document.getElementById('municipiosFirst').appendChild(optGroupMun);
            document.getElementById('municipiosSecond').append(dupNac);
            document.getElementById('municipiosSecond').appendChild(dupMun);
            /////////

            /////////Agrupamos todos los datos en un único array
            dataPyramid = [...data_muni, ...data_global];
